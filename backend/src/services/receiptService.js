...
async function renderPdfInternal(html) {
...
async function renderHTMLToPDF(html) {
  const run = async () => {
    try {
      return await renderPdfInternal(html);
    } catch (error) {
      const message = String(error?.message || error || '');
      if (message.includes('TargetCloseError') || message.includes('Session closed')) {
