...
export function WorkerDashboard() {
  ...
  useEffect(() => { fetchAll(); }, [activeAddress]);
  ...
  const stats = [
    {
      ...
      sub: `${payments.length} milestone${payments.length !== 1 ? 's' : ''} completed`,
  ...
  const tabs = [
    ...
    { id: 'certs',    label: '🎖️ Certificates', count: payments.filter(p => p.assetId).length },
  ] as const;
